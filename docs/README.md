# Documentation

Open these first:

1. [protocol.md](protocol.md) for the end-to-end loop.
2. [architecture-map.md](architecture-map.md) for the public component map.
3. [skill-flow.md](skill-flow.md) for the route-aware public skill flow.
4. [oz-start.md](oz-start.md) for route-aware `/oz-start`.
5. [ybr-route-selection.md](ybr-route-selection.md) for YBR receipt law and route candidates.
6. [candidate-ledgers.md](candidate-ledgers.md) for route and upgrade ledgers.
7. [domain-producers.md](domain-producers.md) for producer stack routing.
8. [scoring-rubric.md](scoring-rubric.md) for scores, thresholds, and grades.
9. [receipt-completeness.md](receipt-completeness.md) for proof quality.
10. [countermap-receipts.md](countermap-receipts.md) for explicit counter standards.
11. [session-continuity.md](session-continuity.md) for the failure inbox, checkpoints, and handoff briefs.
12. [failure-radar.md](failure-radar.md) for the six-mode v0.2 doctrine and counter budgets.
13. [forward-gate.md](forward-gate.md) for the pre-mission go/no-go judgment gate.
11. [ozreceipt.md](ozreceipt.md) and [ozledger.md](ozledger.md) for receipt memory.
12. [receipt-graph.md](receipt-graph.md) for receipt memory as graph nodes.
13. [failure-radar.md](failure-radar.md) for mode selection and failure prediction.
14. [bug-cognizant.md](bug-cognizant.md) for future-build architecture prediction.
15. [public-release-checklist.md](public-release-checklist.md) before public push.

Reference:

- [protocol-contract.md](protocol-contract.md)
- [../commands/README.md](../commands/README.md)
- [../skills/README.md](../skills/README.md)
- [../schemas/README.md](../schemas/README.md)
- [../wiki/Home.md](../wiki/Home.md)
